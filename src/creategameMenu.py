...
class CreateGameMenu(start):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.Menu_Game.draw_text('CreateGame', 30, self.Menu_Game.screen.get_width() / 2, self.Menu_Game.screen.get_height() / 2 - 40)
            self.Menu_Game.draw_text('Select number of players:', 20, self.instructionx, self.instructiony)
            self.Menu_Game.draw_text("2 Players", 20, self.twoplayerx, self.twoplayery)
            self.Menu_Game.draw_text("3 Players", 20, self.threeplayerx, self.threeplayery)
            self.Menu_Game.draw_text("4 Players", 20, self.fourplayerx, self.fourplayery)
            self.draw_cursor()
    ...
    def check_how_many_players(self):
        if self.Menu_Game.START_KEY:
            ...
            for _ in range(5):
                ...
                random_string += (chr(random_integer).upper())
            if self.state == '2 player':
                self.Menu_Game.number_of_players = 2
                random_string = str(self.Menu_Game.number_of_players) + random_string
                self.Menu_Game.game_code = random_string
                self.Menu_Game.curr_menu = self.Menu_Game.waiting_screen
            elif self.state == '3 player':
                self.Menu_Game.number_of_players = 3
                random_string = str(self.Menu_Game.number_of_players) + random_string
                self.Menu_Game.game_code = random_string
                self.Menu_Game.curr_menu = self.Menu_Game.waiting_screen
            elif self.state == '4 player':
                self.Menu_Game.number_of_players = 4
                random_string = str(self.Menu_Game.number_of_players) + random_string
                self.Menu_Game.game_code = random_string
                self.Menu_Game.curr_menu = self.Menu_Game.waiting_screen
            self.run_display = False
